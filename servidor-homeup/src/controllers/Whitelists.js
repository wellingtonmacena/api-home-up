...
module.exports = {
    ...
    async create(req, res) {
        ...
        try {
            ...
            if (response.length > 0) {
                return res.json({ message: "Funcionário já possui essa aplicação em sua whitelist" })
            }

            else {
                response = await connection('whitelist')
                    .insert({
                        aplicacao,
                        fkFuncionario
                    })
    ...
    async delete(req, res) {
        ...
        try {
            ...
            return res.json({ message: "Aplicação foi excluida da whitelist do funcionario" })
        }
        catch (err) {
